...
void PuyoConnectSend::Init(){
    NetHandle = -1;
    //192.168.  1.  15    ←くさもちpcの　IPv4
    //127.  0.  0.  1     ←自分を指すIPv4
    //25.11.194.136       ←hamachi くさもちのIPv4
    //25.41.213.206       ←hamachi あさだのIPv4
  //接続先のIPアドレスを指す
    Ip.d1 = 127;
    Ip.d2 = 0;
    Ip.d3 = 0;
...
bool PuyoConnectSend::Connect(){
    ...
    if (NetHandle != -1) {
        return true;
#ifdef _DEBUG
        DrawFormatString(100, 100, 0xffffff, "通信接続成功");
#endif
    }
    else {
#ifdef _DEBUG
        DrawFormatString(100, 100, 0xffffff, "通信接続中");
#endif
    }
    return false;
}
